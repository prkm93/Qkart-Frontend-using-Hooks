...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // resetting the new Address state
        ...
        // setting addresses with newly added address
        setAddresses((currAddress) => ({
          ...currAddress,
          all: response.data
        }));
  ...
  const validateRequest = (items, addresses) => {
    ...
    if (Number(balance) < getTotalCartValue(items)) {
    ...
    try {
      if (validateRequest(items, addresses)) {
        ...
        if (response.status === 200 && response.data.success) {
          const balance = localStorage.getItem('balance');
          localStorage.setItem('balance', Number(balance) - getTotalCartValue(items));
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               {
                 addresses && addresses.all.length ?
                 addresses.all.map((item) => {
                   return (
                    <Box
                      ...
                      alignItems="center"
                      className={`${item._id === addresses.selected ? "address-item selected":"address-item not-selected"}`}
                      key={item._id}
                      onClick={() => setAddresses((currAddress) => ({
                                        ...currAddress, 
                                        selected: item._id
                                     })
                              )}
                    >
                      <Typography my="1rem" alignSelf="center" id={item._id}>
                        {item.address}
                      </Typography>

                      <Typography my="1rem" alignSelf="center" id={item._id}>
                        <Button variant="text" startIcon={<Delete/>} onClick={() => deleteAddress(token, item._id)}>Delete</Button>
                      </Typography>
                    </Box> 
                   );
                 })
